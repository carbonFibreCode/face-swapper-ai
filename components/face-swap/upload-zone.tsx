            ...
            {file && preview ? (
                <div className="absolute inset-0 w-full h-full bg-black group">
                    {file.type.startsWith("image") ? (
                        ...
                        <video src={preview} className="w-full h-full object-cover shadow-inner" controls={false} muted />
                    )}
                    <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity text-white text-[10px] font-bold uppercase tracking-widest">
                        ...
                        <span className="text-[8px] opacity-60 mt-1 font-normal italic">{(file.size / 1024 / 1024).toFixed(1)} MB</span>
                    </div>
                </div>
            ) : (
                <div className="flex flex-col items-center justify-center gap-3 py-4">
                    <div className={cn(
                        ...
                        isDragActive ? "bg-primary text-primary-foreground scale-110 shadow-xl" : "bg-muted text-muted-foreground group-hover:bg-primary/10 group-hover:text-primary"
                    )}>
