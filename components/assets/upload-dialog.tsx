...
export function UploadDialog({ children }: UploadDialogProps) {
    ...
    const validateVideo = async (file: File): Promise<{ valid: boolean; duration: number; error?: string }> => {
        return new Promise((resolve) => {
            ...
            video.onloadedmetadata = () => {
                ...
                if (video.duration < 3) {
                    resolve({ valid: false, duration: video.duration, error: 'Video must be at least 3 seconds long' });
                } else {
